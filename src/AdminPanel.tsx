...
이 유저 댓글에서 파생된 연애/데이트 고민, MBTI 성향 차이, 서운함 또는 억울한 썰을 주제로 삼아, 다른 스레드 유저들이 또다시 폭발적으로 참견하고 댓글을 달 수밖에 없는 새로운 스레드 포스팅 1개를 작성해주세요!`;
        ...
        topicInstruction = `[카테고리: 둘픽(Dulpick) 앱 자연스러운 소개 및 관심 유도]
- 배경/페인포인트: 데이트할 때 인스타 릴스 저장만 수백 개 해두고 막상 어디 갈지 못 찾거나, 매번 한 사람만 코스 찾고 고민하느라 지쳤던 찐고민
- 해결책 (둘픽 앱 소개): "그래서 우리가 직접 인스타 릴스 링크만 공유하면 지도에 장소가 자동 저장되고, 둘이 장바구니처럼 같이 장소 담아서 데이트 코스 짤 수 있는 '둘픽'이라는 앱을 만들었거든!"
- 핵심 톤앤매너:
  * "이러이러한 기능이 있는 '둘픽' 앱을 직접 만들었는데 관심 있는 사람 있을까?" 하는 솔직하고 친근한 톤
  * 절대 딱딱한 광고나 마케팅 문체 금지 ("혁신적인 앱", "지금 다운로드", "많은 이용 부탁드립니다" 등 금지)
  * 20대 메이커 커플이 스레드 친구들에게 편하게 털어놓듯 100% 일상 구어체 반말(~어, ~거든, ~있을까?)
- 마무리: "혹시 우리처럼 이런 거 필요했던 사람 있어? 관심 있는 사람 댓글 달아주면 링크 보내줄게!", "이런 거 있으면 써볼 사람 있어?" 등 유저의 반응을 묻는 질문으로 마무리`;
      } else {
        topicInstruction = '주제: 1년차 커플의 데이트 코스 짜기 팁이나, 요즘 2030이 좋아하는 데이트 장소 특징 등 연애/데이트 관련 흥미로운 썰';
      }

      const prompt = category === 'service' ? `
당신은 데이트 장소 정하기가 너무 답답해서 직접 커플 앱을 만든 20대 메이커 커플입니다.
스레드(Threads)에 올릴 자연스럽고 매력적인 서비스 소개 글 초안을 작성해주세요.
${topicInstruction}

[필수 작성 규칙]
1. 반드시 서비스명 '둘픽(Dulpick)'과 핵심 기능(인스타 릴스 링크 공유 시 지도 자동 저장, 커플이 함께 장소 담아 코스 짜기)을 자연스럽게 언급하세요.
2. "이러이러한 기능이 있는 '둘픽' 앱을 직접 만들었는데 관심 있는 사람 있을까?" 하는 뉘앙스로 작성하세요.
3. 절대 딱딱한 광고나 홍보 문체처럼 쓰지 마세요. 스레드에서 편하게 읽히는 100% 일상 구어체 반말(~어, ~거든, ~있을까?)을 사용하세요.
4. 모바일 화면에서 한눈에 읽히도록 2~3줄 단위로 적절한 줄바꿈을 넣으세요.
5. 마지막 문장은 관심 있는 사람의 댓글이나 피드백을 물어보는 자연스러운 질문("관심 있는 사람 댓글 달아주면 링크 보내줄게!", "이런 거 있으면 데이트할 때 써볼 사람 있어?")으로 마무리하세요.
6. 해시태그는 절대 넣지 마세요.
` : `
